            ...
            <table class="min-w-full bg-white dark:bg-gray-800 rounded shadow text-sm sm:text-base">
                <thead>
                    <tr class="bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200">
                        ...
                        <th class="px-2 py-1 sm:px-4 sm:py-2">Rol</th>
                ...
                <tbody>
                    @foreach($users as $user)
                        <tr class="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-900">
                            ...
                            <td class="px-2 text-white py-1 sm:px-4 sm:py-2">{{ $user->role }}</td>
                            <td class="px-2 text-white py-1 sm:px-4 sm:py-2 flex space-x-2">
                                <a href="{{ route('users.edit', $user) }}" class="text-blue-600 hover:underline">Editar</a>
                                <!-- Modal Trigger -->
                                <button type="button" class="text-red-600 hover:underline ml-2" onclick="document.getElementById('delete-modal-{{ $user->id }}').style.display='flex'">Eliminar</button>
                                <!-- Modal -->
<div id="delete-modal-{{ $user->id }}" class="fixed inset-0 z-50 items-center justify-center bg-black bg-opacity-40 hidden overflow-y-auto" style="display:none;">
                                    <div class="bg-white rounded-lg shadow-lg p-6 mx-auto w-full max-w-sm flex flex-col items-center relative">
                                        <button type="button" class="absolute top-2 right-2 text-red-600 text-2xl font-bold" onclick="document.getElementById('delete-modal-{{ $user->id }}').style.display='none'">&times;</button>
                                        <div class="mb-4">
                                        ...
                                        <h3 class="text-lg font-bold mb-2 text-gray-900">¿Estás seguro de borrar este usuario?</h3>
                                        <p class="text-gray-700 mb-6 text-center">Esta acción no se puede deshacer.</p>
                                        <div class="flex w-full justify-between">
                                            <form action="{{ route('users.destroy', $user) }}" method="POST" class="w-1/2">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="w-full bg-red-600 text-white px-4 py-2 rounded font-bold hover:bg-red-700">Aceptar</button>
                                            </form>
                                            <button type="button" class="w-1/2 ml-2 bg-gray-200 text-gray-700 px-4 py-2 rounded font-bold hover:bg-gray-300" onclick="document.getElementById('delete-modal-{{ $user->id }}').style.display='none'">Cancelar</button>
                                        </div>
                                    </div>
                                </div>
                            </td>
